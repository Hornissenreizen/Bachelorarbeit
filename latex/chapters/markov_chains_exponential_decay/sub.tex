...
\subsubsection{The Defective Case}
    ...
    \begin{align*}
        ...
        \begin{bmatrix}
        1 & 0 & 0 & 0 & 0 \\
        0 & \lambda_2^k & \binom{\tau}{1} \lambda_2^{k - 1} & \binom{k}{2} \lambda_2^{k - 2} & 0 \\
        ...
        \quad . \\
    \end{align*}
    Thus, at $j geq i, \ d \coloneqq j - i$, $(\bm{D}^k)_{ij} = \binom{k}{d} \lambda_2^{k - d}$, and hence we have
    \begin{align*}
        (\bm{D}^{k + 1})ij &= (\bm{D}^k)_{i, i - (d - 1)} (\bm{D})_{i - (d - 1), j} + (\bm{D}^k)_{i, i - d} (\bm{D})_{i - d, j} \\
        ...
        &= \binom{k + 1}{d} \lambda_2^{k + 1 - d} \quad ,
    \end{align*}
    just as expected.
\end{document}