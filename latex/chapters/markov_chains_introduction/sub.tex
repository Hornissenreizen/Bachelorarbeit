...
\begin{lemma}[Period is a Class Property]
    \label{lemma:period_is_a_class_property}
...
\vspace{-2.5em}
\begin{proof}
    Since $i \leftrightsquigarrow j$, there exist integers $m, n > 0$ such that $(\bm{P}^m)_{ij} > 0$ and $(\bm{P}^n)_{ji} > 0$. Consider any time $t$ such that $(\bm{P}^t)_{ii} > 0$. By combining paths it follows that
    \[
        (\bm{P}^{n + t + m})_{jj} \geq (\bm{P}^n)_{ji} \cdot (\bm{P}^t)_{ii} \cdot (\bm{P}^m)_{ij} > 0 \quad .
    \]
    Define $\mathcal{T}_i := \{ t : (\bm{P}^t)_{ii} > 0 \}$, $\mathcal{T}_j := \{ t : (\bm{P}^t)_{jj} > 0 \}$. Consider $\mathcal{T}' \coloneqq \dfrac{\mathcal{T}_i}{\operatorname{period}(i)}$.
    
    Clearly, $\gcd \mathcal{T}' = 1$. Hence, there exists $\ell \in \mathcal{T}'$ s.t. $(\ell + 1) \in \mathcal{T}'$ (as $\mathcal{T}'$ can be thought of as arising from an irreducible aperiodic Markov chain and hence lemma~\ref{lemma:aux} is applicable). Thus, $(\operatorname{period}(i)\ell) \in \mathcal{T}_i$ and $(\operatorname{period}(i)\ell + \operatorname{period}(i)) \in \mathcal{T}_i$.

    It follows that
    \[
        \{m + n + \operatorname{period}(i) \ell, \ m + n + \operatorname{period}(i) \ell + \operatorname{period}(i)\} \subset \mathcal{T}_j \quad .
    \]
    Thus, $\operatorname{period}(j) = \gcd \mathcal{T}_j \leq \operatorname{period}(i)$. Because of symmetry, we also have $\operatorname{period}(i) \leq \operatorname{period}(j)$. Ultimately, we have $\operatorname{period}(i) = \operatorname{period}(j)$.
\end{proof}
